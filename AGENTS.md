...
- V3 is the current authorized implementation version. Its approved detailed design is at `docs/architecture/v3-knowledge-base-maintenance-design.md`; implement it in the documented slices, starting with the read-only health core and preserving the V1/V2 baselines.
...
- Treat V1 and V2 as completed baselines, not as unfinished MVP delivery. V3 implementation is authorized only within the committed detailed design and must preserve those baselines.
- Implement V3 in the ordered, independently verifiable slices defined by the approved design. Do not pull migrations, chat integration, action refactoring, repair actions, reliable dispatch, or frontend work into an earlier slice unless that slice requires it to remain buildable and testable.
...
- Describe V1 and V2 as completed baselines, V3 as the bounded current implementation version, and V4+ only as goals and entry conditions.
