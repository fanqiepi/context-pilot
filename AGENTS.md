...
- V4's first-phase `DOCUMENT_COMPARISON` contract and complete Slice 2-7 production implementation are authorized and implemented with local engineering gates passing. Slice 1 evidence demonstrates a need for scoped multi-step retrieval but does not justify a model Planner, so the production baseline uses versioned deterministic dimension detection and per-document orchestration. Preserve the implementation and independently verifiable slices documented in `docs/architecture/v4-knowledge-research-design.md`; the slices are engineering checkpoints rather than separate permission gates. The DeepSeek/DashScope production end-to-end release gate must still be recorded before describing V4 as fully released. Do not introduce production agents, model planning, LangGraph or other workflow engines, MCP integration, microservices, message queues, multi-tenancy, plugin systems, dynamic tools, or model-generated SQL under the current scope.
...
- V3 was completed through the ordered, independently verifiable slices in the approved design. V4 Slice 1 is complete with reviewable evidence, and the complete first-phase production implementation (Slices 2-7) is authorized under the fixed deterministic orchestration contract. Preserve the slice order and verification gates without requesting repeated authorization between slices.
...
- Describe V1, V2, and V3 as completed baselines, V4 as a detailed design with Slice 1 completed and Slices 2-7 authorized for production implementation, and V5+ only as goals and entry conditions.
