...
ContextPilot 是一个基于 Spring AI 的学习型知识库与任务助手。功能型 MVP 已完成，采用前后端分离的模块化单体架构，提供文档摄取、检索增强生成、引用、流式回答、会话历史、调用记录和单向“有用”反馈。

当前已授权的下一阶段是固定能力路由和首个需人工确认的创建知识库操作。规划采用显式 Java 应用服务、持久化操作提案和确认后执行，不引入 Agent、LangGraph、动态插件或通用工具平台；规划接口在实际代码和 OpenAPI 落地前不可调用。
