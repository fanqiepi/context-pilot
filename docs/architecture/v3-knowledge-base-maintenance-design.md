...
- 前端卡片展示总体状态、`dataAsOf`、完整性、当前 profile、文档计数、异常依据、建议动作和不可执行原因；切片 5 已开放失败文档的重试提案按钮，索引重建按钮仍留给切片 6。后端聚焦测试、真实 PostgreSQL HTTP/SSE 链路、前端类型检查和生产构建已通过。
...
- `RetryDocumentProcessingActionExecutor` 在确认取得执行权后复用单文档重试原语，再次校验 `FAILED`、处理开关和次数上限；陈旧、删除、关闭或超限目标进入动作 `FAILED` 且不修改文档，重复及并发确认最多提交一次。前端只为可操作的失败文档显示生成重试提案按钮，重复请求恢复同一操作卡片；`REINDEX_DOCUMENT` 仍明确等待切片 6，事务提交后派发、队列拒绝保留 `PENDING` 和恢复扫描仍等待切片 7。
- 切片 6 至切片 8 尚未实施，下一步是索引重建提案及 profile、实际向量和 VectorStore 可用性的双重实时校验。
