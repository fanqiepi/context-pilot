...
- 切片 2 已通过真实 PostgreSQL/pgvector 迁移及持久化测试，覆盖来源快照不可变、稳定排序、重复创建冲突、跨知识库拒绝和历史回显；尚未新增公开报告端点、健康聊天路由或 SSE 事件。
- 切片 3 至切片 8 尚未实施，下一步是确定性健康路由、SSE `health_report`、公开报告查询和前端卡片刷新恢复。
