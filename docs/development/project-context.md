...
项目按大版本演进。V1 功能型 RAG 基线、V2 可控行动助手和 V3“知识库健康检查与维护助手”均已完成；V2 完成了首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作，V3 完成了可恢复、可审计的健康检查与单文档维护闭环。V4 受限 Plan-and-Execute 第一阶段合同已确认，Slice 1 评估集与固定离线基线已经完成，生产实现与 Slice 2 仍未授权；显式记忆、MCP 等外部工具互操作和多 Agent 模式仍属于更远期方向。远期路线和设计草案都不自动代表开发授权。
...
- 2026-08-17 V4 Slice 1 已完成 7 份合成文档、32 个语义案例、12 个生命周期定义、单轮 Top-K 对照、测试侧候选编排原型和 DeepSeek 代表性对照。固定多步子集上候选原型为 12/12，单轮 Top-K 5 为 0/12；8 个代表案例各重复 3 次的真实模型运行中，候选多步为 9/9、单轮为 0/9，计划、引用、隔离和预算均通过，但候选 Token 为单轮 2.64 倍、平均延迟为 3.25 倍。结果证明范围化多步检索的必要性，仍未证明模型 Planner 优于固定编排。完整结果见 `evals/reports/2026-08-17-v4-document-comparison-slice1.md`；Slice 2 及生产实现仍需另行讨论和授权。
...
- **V4：知识研究助手（Slice 1 评估已完成）**。第一阶段只做显式多文档比对；评估集和固定离线基线已经建立，在 `KNOWLEDGE_QA` 内落地任何生产级研究编排仍需后续切片单独授权，范围与进入条件见 `docs/architecture/v4-knowledge-research-design.md`。
...
- V4 Slice 1 已完成且当前没有后续切片授权。可以维护既有版本化评估语料、配置、测试侧候选原型和脱敏报告，但不得把研究能力加入生产 `src/main`、Flyway、正式 API/SSE 或前端入口。Slice 2 及以后必须依据 Slice 1 证据逐片讨论、更新文档并授权；Agent 循环、LangGraph、MCP 和动态工具继续禁止。
