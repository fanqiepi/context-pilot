...
项目按大版本演进。V1 功能型 RAG 基线、V2 可控行动助手和 V3“知识库健康检查与维护助手”均已完成；V2 完成了首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作，V3 完成了可恢复、可审计的健康检查与单文档维护闭环。V4 受限 Plan-and-Execute 第一阶段合同已确认，但尚未授权实施；显式记忆、MCP 等外部工具互操作和多 Agent 模式仍属于更远期方向。远期路线和设计草案都不自动代表开发授权。
...
- 2026-08-17 V4 第一阶段合同已确认：只做显式 `DOCUMENT_COMPARISON`，选择 2 至 5 份合格文档，计划自动执行并可取消；最多 4 个顺序步骤、按文档检索、90 秒硬超时，使用四张研究表和可信证据引用，断线继续、重启失败收敛。当前仍需评估证据与实施授权，尚未允许生产实现。
...
- **V4：知识研究助手（第一阶段合同已确认，未授权实施）**。只做显式多文档比对；在 `KNOWLEDGE_QA` 内使用受限 Plan-and-Execute 形成带引用结果，范围与进入条件见 `docs/architecture/v4-knowledge-research-design.md`。
...
- V4 当前只授权文档与评估设计。第一阶段主要合同已在详细设计第 19 节确认，但在第 20 节进入条件满足前，不得实现 Flyway 迁移、API、前端入口或运行时，也不得引入 Agent 循环、LangGraph、MCP 或动态工具。
