...
- 2026-08-15 已批准 V3 详细设计及其六项默认决策并授权实施。切片 1 至切片 5 已完成：健康只读核心和 V11 不可变报告已接入 `KNOWLEDGE_QA/v2` 完整句路由、同步聊天、SSE `health_report`、公开报告查询与前端报告卡片；V13 已把动作核心重构为 sealed 强类型参数、JSONB 静态编解码和显式分派；失败文档现在可从可信报告明细生成单文档重试提案，并在生成与确认时双重实时校验。V14 增加 `HEALTH_REPORT_ISSUE_SELECTED` 消息依据。下一步是切片 6 索引重建提案；事务提交后派发与恢复扫描仍留给切片 7。
