...
项目按大版本演进。V1 功能型 RAG 基线、V2 可控行动助手和 V3“知识库健康检查与维护助手”均已完成；V2 完成了首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作，V3 完成了可恢复、可审计的健康检查与单文档维护闭环。V4 以后才可能研究有限规划、显式记忆、MCP 等外部工具互操作和多 Agent 模式。远期路线和设计草案都不自动代表开发授权。

## 当前状态

- 已完成工程基线、PostgreSQL/pgvector 与 Flyway、知识库 CRUD，以及文档上传、异步解析切分、状态与有限重试、pgvector 索引、知识库隔离检索和逻辑删除后的向量清理。
- 前端已实现知识库与文档管理、文档处理状态轮询、独立聊天、SSE 增量回答、引用展示、健康报告与操作卡片和会话历史恢复；类型检查、生产构建与本地真实后端页面级验收均已通过。
...
- 2026-08-15 V3 切片 1 至切片 8 全部完成：健康只读核心和 V11 不可变报告已接入 `KNOWLEDGE_QA/v2` 完整句路由、同步聊天、SSE `health_report`、公开报告查询与前端报告卡片；V13 已把动作核心重构为 sealed 强类型参数、JSONB 静态编解码和显式分派；失败文档重试及来源未知、profile 过期或当前 profile 向量缺失的索引重建，均可从可信报告明细生成单文档提案，并在生成与确认时双重实时校验。V14 增加 `HEALTH_REPORT_ISSUE_SELECTED` 消息依据。文档任务已统一为事务提交后派发，执行器拒绝时保持 `PENDING`，应用启动与低频有界扫描负责重新派发。固定评估、全量回归、前端构建、真实接口与页面目检均已通过，V3 正式完成。
...
- V3 已按详细设计的八个切片完成并通过验收。已完成基线仍限定单文档动作、不可变报告、实际向量检查、报告明细入口、sealed 强类型动作参数、静态分派以及提交后派发与有界恢复扫描；任何扩展仍需新的版本授权。完整路线见 `docs/requirements/product-requirements.md`。
