...
项目按大版本演进。V1 功能型 RAG 基线和 V2 可控行动助手均已完成；V2 完成了首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作。当前没有获准实施的新大版本，V3 只保留“知识库健康检查与维护助手”的场景雏形，V4 以后才可能研究有限规划、显式记忆、MCP 等外部工具互操作和多 Agent 模式。远期路线不代表当前开发授权。
...
- **V3：知识库维护助手（场景雏形，未授权实施）**。面向“检查知识库健康状态并协助修复”场景，候选固定能力包括健康检查、失败文档重试和过期索引重建；只有显式批准 V3 范围后才细化合同。
...
- V3 及以后只是规划候选；当前不得实施。开始新大版本前必须显式批准范围，并同步 `AGENTS.md` 及需求、架构、数据、API 和评估文档。完整路线见 `docs/requirements/product-requirements.md`。
