...
项目按大版本演进。V1 功能型 RAG 基线已经完成；V2 是当前唯一获准实施的版本，目标是完成首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作。V3 只保留“知识库健康检查与维护助手”的场景雏形，V4 以后才可能研究有限规划、显式记忆、MCP 等外部工具互操作和多 Agent 模式。远期路线不代表当前开发授权。
...
- `CREATE_KNOWLEDGE_BASE` 已完成强类型参数解析、持久化操作提案、独立查询/确认/拒绝接口、过期处理、原子幂等执行、SSE `action_required` 事件、历史恢复和前端操作卡片；重复及并发确认、拒绝、过期和名称冲突已有真实 PostgreSQL 自动化测试，确认、取消和刷新恢复已通过本地真实页面验收。2026-08-14 已建立并通过首份固定路由与操作安全基线；V2 版本完成结论留待整体变更审阅后确认。
...
- **V2：可控行动助手（当前授权）**。只完整实现 `CREATE_KNOWLEDGE_BASE` 的强类型提案、持久化、确认/拒绝、过期、原子幂等执行、审计和可恢复前端卡片。
- **V3：知识库维护助手（场景雏形，未授权实施）**。面向“检查知识库健康状态并协助修复”场景，候选固定能力包括健康检查、失败文档重试和过期索引重建；V2 完成后再细化合同。
...
- V2 必须保留已有 RAG、知识库隔离、拒答、引用、SSE、历史和反馈行为；任何新增能力不得破坏这些基线。
...
- V3 及以后只是规划候选；开始实施前必须重新确认范围并同步 `AGENTS.md`。完整路线见 `docs/requirements/product-requirements.md`。
