...
项目按大版本演进。V1 功能型 RAG 基线和 V2 可控行动助手均已完成；V2 完成了首个需人工确认的 `CREATE_KNOWLEDGE_BASE` 操作。当前没有获准实施的新大版本；V3 已形成“知识库健康检查与维护助手”详细设计草案，但尚未授权实现。V4 以后才可能研究有限规划、显式记忆、MCP 等外部工具互操作和多 Agent 模式。远期路线和设计草案都不自动代表开发授权。
...
- 2026-08-15 已形成 V3 详细设计草案：使用不可变健康报告主表与异常明细表保存 `dataAsOf`、完整性、来源和修复建议；动作核心计划演进为静态强类型多动作协议，并在提案生成和确认执行时双重实时校验。该草案尚未授权实现。
...
- **V3：知识库维护助手（详细设计草案，未授权实施）**。面向“检查知识库健康状态并协助修复”场景，固定候选能力包括健康检查、失败文档重试和过期或缺失索引重建；详细设计见 `docs/architecture/v3-knowledge-base-maintenance-design.md`。
...
- V3 详细设计仍是规划候选；当前不得实施生产代码、迁移或前端功能。开始实现前必须显式批准设计与范围，并同步 `AGENTS.md` 及需求、架构、数据、API 和评估文档的实施状态。完整路线见 `docs/requirements/product-requirements.md`。
