...
当前已授权进入“固定能力路由 + 受控业务操作”阶段，但仍不引入 Agent、LangGraph 或其他工作流引擎、微服务、消息队列、多租户、登录权限、独立向量数据库、插件系统或 OCR。
...
- 产品决策将当前版本视为功能型 MVP 完成；前端真实页面验收、固定评估基线和发布前加固转为延期事项，不阻塞下一阶段开发，也不应被误写为已完成。
- 下一阶段已授权实现 `SIMPLE_CHAT`、`KNOWLEDGE_QA`、`BUSINESS_ACTION` 三类固定能力路由，以及首个需要人工确认的 `CREATE_KNOWLEDGE_BASE` 业务操作；当前代码尚未实现这些能力。
- 日常开发和集成分支为 `develop`，不得直接提交到 `main`。

## 路线图边界

- 当前版本保留已有 RAG、知识库隔离、拒答、引用、SSE、历史和反馈行为；下一阶段不得以路由改造为由破坏这些能力。
